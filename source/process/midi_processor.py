...
class MIDIProcessor:
            ...
            if token < len(self.event_types):
                ...
                if self.min_pitch <= pitch <= self.max_pitch:
                    ...
                    if i + 1 < len(tokens):
                        ...
                        if next_token >= len(self.event_types) + (
                            self.max_pitch - self.min_pitch + 1
                        ):
                            ...
                            events.append({"type": "note_on", "pitch": pitch, "velocity": velocity})
                            i += 2
                        else:
                            # This is a note_off
                            events.append({"type": "note_off", "pitch": pitch})
                            i += 1
                    else:
    ...
    def process_midi_file(self, midi_file: str) -> Optional[Dict[str, Any]]:
        ...
        result = {
            ...
            "metadata": {"duration": end_time, "tempo": tempo, "instruments": instruments},
        }
    ...
    def save_processed_data(self, processed_data: List[Dict[str, Any]], output_file: str):
        ...
        print(f"Saved {len(processed_data)} processed MIDI files to {output_file}")
