"""
Model Trainer - Training utilities for Music Transformer
"""
...
class ModelTrainer:
    ...
    def train_epoch(self) -> float:
        ...
        for batch in progress_bar:
            ...
            # Prepare text embeddings (use BERT if available, otherwise TF-IDF)
            text_embeddings = bert_embedding
            if bert_embedding.sum() == 0:  # If BERT embeddings are zero
                text_embeddings = tfidf_features.unsqueeze(1).expand(-1, 512, -1)
            
            # Forward pass
            logits = self.model(
                midi_tokens[:, :-1],  # Input sequence
                text_embeddings
            )
...
def enhanced_train_model(data_file: str, 
                        output_dir: str = "models/checkpoints",
                        **kwargs) -> ModelTrainer:
    """Enhanced training function with automatic data loading."""
    from ..process.data_preparer import DataPreparer
    
    # Load and prepare data
    preparer = DataPreparer()
    training_data = preparer.prepare_training_data(data_file, output_dir.replace('/checkpoints', '/processed'))
