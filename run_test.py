...
from source.test import EvaluationMetrics, ModelEvaluator, ModelTester


def main():
    parser = argparse.ArgumentParser(
        description="Test model performance and evaluate generated music"
    )
    ...
    if args.generated_files:
        ...
        if valid_files:
            evaluation_results = evaluator.evaluate_batch(
                generated_files=valid_files, reference_files=args.reference_files
            )
    ...
    if args.benchmark and args.model_path:
        ...
        if benchmark_results["success"]:
            ...
            print(
                f"  Average sequence length: {benchmark_results.get('avg_sequence_length', 0):.1f}"
            )
        else:
        ...
        for file in args.generated_files:
            if os.path.exists(file):
                try:
                    ...
                    detailed_metrics.append({"file": file, "metrics": metrics_result})
